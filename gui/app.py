"""
Main Application Window - Complete GUI Implementation
"""
...
from core.settings import colors, current_theme, app_settings, save_settings, added_vehicles, THEMES, ADDED_VEHICLES_FILE, DEFAULT_THEMES, EDITABLE_COLOR_KEYS, COLOR_LABELS, reset_theme_colors, update_theme_color
from core.updater import CURRENT_VERSION
from core.file_ops import create_vehicle_folders, delete_vehicle_folders, edit_material_json
from utils.debug import setup_universal_scroll_handler, toggle_debug_mode
...
try:
    from utils.file_ops import generate_mod
except ImportError:
    try:
        from core.file_ops import generate_mod
    except ImportError:
        print("[WARNING] generate_mod not found in file_ops, using fallback")
        def generate_mod(*args, **kwargs):
            messagebox.showerror("Error", "generate_mod function not available")
...
def show_hover_preview(carid, x, y):
    ...
    try:
        ...
        vehicle_name = VEHICLE_IDS.get(carid, carid)
        header_text = f"Name: {vehicle_name} | ID: {carid}"
        ctk.CTkLabel(header, text=header_text, text_color=colors["accent_text"], font=("Segoe UI", 11, "bold")).pack()
...
steam_icon_white = None
steam_icon_black = None
folder_icon_white = None
folder_icon_black = None
steam_radio_sidebar = None
custom_radio_sidebar = None
steam_icon_label = None
custom_icon_label = None
sidebar_search_var = ctk.StringVar()
expanded_vehicle_carid = None  # Track which vehicle is expanded

def load_output_icons():
    """Load output icons for both themes"""
    global steam_icon_white, steam_icon_black, folder_icon_white, folder_icon_black
    ...
    try:
        ...
        if os.path.exists(steam_white_path):
            ...
            print(f"[WARNING] Steam white icon not found at: {steam_white_path}")
        
        if os.path.exists(steam_black_path):
            steam_icon_black = ctk.CTkImage(
            ...
            print(f"[WARNING] Steam black icon not found at: {steam_black_path}")
        ...
        if os.path.exists(folder_white_path):
            folder_icon_white = ctk.CTkImage(
            ...
            print(f"[WARNING] Folder white icon not found at: {folder_white_path}")
        
        if os.path.exists(folder_black_path):
            folder_icon_black = ctk.CTkImage(
            ...
            print(f"[WARNING] Folder black icon not found at: {folder_black_path}")
            
    except Exception as e:
        print(f"[ERROR] Failed to load output icons: {e}")

def update_output_icons():
    """Update icon labels based on current theme"""
    global steam_icon_label, custom_icon_label
    
    if steam_icon_label and custom_icon_label:
        if current_theme == "dark":
            steam_icon = steam_icon_white
            folder_icon = folder_icon_white
        else:
            steam_icon = steam_icon_black
            folder_icon = folder_icon_black
        
        if steam_icon:
            steam_icon_label.configure(image=steam_icon)
        
        if folder_icon:
            custom_icon_label.configure(image=folder_icon)
        
        print(f"[DEBUG] Updated output icons for {current_theme} theme")
...
for btn_text, view_name in [("Project", "generator"), ("How to Use", "howto"), 
...
steam_icon_label = ctk.CTkLabel(
    steam_option_sidebar,
    text="",
    image=None
)
steam_icon_label.pack(side="left", padx=(10, 5), pady=10)
...
steam_radio_sidebar.pack(side="left", padx=0, pady=10)
...
custom_icon_label = ctk.CTkLabel(
    custom_option_sidebar,
    text="",
    image=None
)
custom_icon_label.pack(side="left", padx=(10, 5), pady=10)
...
custom_radio_sidebar.pack(side="left", padx=0, pady=10)
...
def select_vehicle(carid):
    ...
    vehicle_display_var.set(display_name) 
...
def refresh_project_display():
    """Refresh the Vehicles in project display"""
...
ctk.CTkLabel(generator_scroll, text="Vehicles in project", font=ctk.CTkFont(size=14, weight="bold"), text_color=colors["text"]).pack(anchor="w", padx=10, pady=(10,5))
    ...
    try:
        ...
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            ...
            if not found_files:
                ...
                print(f"[DEBUG] Files with 'skin' in name: {skin_files[:10]}") 
...
settings_canvas = ctk.CTkCanvas(settings_tab, bg=colors["app_bg"], highlightthickness=0)
settings_scrollbar = ctk.CTkScrollbar(settings_tab, orientation="vertical", command=settings_canvas.yview)
settings_scrollable_frame = ctk.CTkFrame(settings_canvas, fg_color=colors["app_bg"])
...
settings_window_id = settings_canvas.create_window((0, 0), window=settings_scrollable_frame, anchor="nw")
settings_canvas.configure(yscrollcommand=settings_scrollbar.set)

settings_canvas.pack(side="left", fill="both", expand=True)

def check_settings_scroll(event=None):
    """Only show scrollbar when content exceeds visible area AND resize frame width"""
    canvas_width = settings_canvas.winfo_width()
    if canvas_width > 1:  
        settings_canvas.itemconfig(settings_window_id, width=canvas_width)
    
    settings_canvas.update_idletasks()
    
    bbox = settings_canvas.bbox("all")
    if bbox and bbox[3] > settings_canvas.winfo_height():
        if not settings_scrollbar.winfo_ismapped():
            settings_scrollbar.pack(side="right", fill="y")
            settings_canvas.after(10, lambda: settings_canvas.itemconfig(settings_window_id, width=settings_canvas.winfo_width()))
    else:
        if settings_scrollbar.winfo_ismapped():
            settings_scrollbar.pack_forget()
            settings_canvas.after(10, lambda: settings_canvas.itemconfig(settings_window_id, width=settings_canvas.winfo_width()))

settings_canvas.bind("<Configure>", check_settings_scroll)

settings_canvas.after(100, check_settings_scroll)
...
def toggle_developer_mode():
    ...
    if developer_mode_var.get():
        ...
        # Pack debug_mode_frame right after developer_checkbox
        debug_mode_frame.pack(anchor="w", padx=0, pady=(0,5), after=developer_checkbox)
    else:
        ...
        # Hide debug_mode_frame when developer mode is disabled
        debug_mode_frame.pack_forget()
        if debug_mode_var.get():
            debug_mode_var.set(False)
            toggle_debug_mode(app, colors)

ctk.CTkLabel(settings_scrollable_frame, text="Settings", font=ctk.CTkFont(size=16, weight="bold"), text_color=colors["text"]).pack(anchor="w", padx=10, pady=(10,5))

theme_frame = ctk.CTkFrame(settings_scrollable_frame, fg_color="transparent")
...
# --- Replace the Developer Mode section in app.py (around line 1748) ---
...
developer_checkbox.pack(anchor="w", padx=10, pady=(0,5))

# Debug mode frame - only pack when developer mode is enabled
debug_mode_frame = ctk.CTkFrame(settings_scrollable_frame, fg_color="transparent")

debug_checkbox = ctk.CTkCheckBox(
    debug_mode_frame,
    text="Debug Mode (Opens debug console)",
    variable=debug_mode_var,
    command=lambda: toggle_debug_mode(app, colors)
)
debug_checkbox.pack(anchor="w", padx=20)
...
dark_theme_edit_frame = None
light_theme_edit_frame = None
dark_color_entries = {}
light_color_entries = {}
settings_is_scrollable = False

editors_container = ctk.CTkFrame(settings_scrollable_frame, fg_color="transparent")

def toggle_dark_theme_editor():
    """Toggle dark theme color editor visibility"""
    global dark_theme_edit_frame
    
    print(f"[DEBUG] toggle_dark_theme_editor called, switch value: {dark_edit_switch.get()}")
    
    if dark_edit_switch.get():
        print("[DEBUG] Creating/showing dark theme editor")
        if dark_theme_edit_frame is None:
            create_dark_theme_editor()
        
        # Show container with proper expansion
        editors_container.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        
        # Configure grid for equal distribution
        editors_container.grid_columnconfigure(0, weight=1)
        editors_container.grid_columnconfigure(1, weight=1)
        
        # Always use 50% width (column 0)
        dark_theme_edit_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        
        # If light editor is also open, show it in column 1
        if light_edit_switch.get() and light_theme_edit_frame:
            print("[DEBUG] Both editors open - 50/50 split")
            light_theme_edit_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0))
    else:
        print("[DEBUG] Hiding dark theme editor")
        if dark_theme_edit_frame:
            dark_theme_edit_frame.grid_forget()
        
        # If light editor is still open, keep it at 50% width in column 0
        if light_edit_switch.get() and light_theme_edit_frame:
            print("[DEBUG] Repacking light editor to column 0 (50% width)")
            light_theme_edit_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        else:
            check_container_visibility()
    
    settings_canvas.after(100, check_settings_scroll)

def toggle_light_theme_editor():
    """Toggle light theme color editor visibility"""
    global light_theme_edit_frame
    
    print(f"[DEBUG] toggle_light_theme_editor called, switch value: {light_edit_switch.get()}")
    
    if light_edit_switch.get():
        print("[DEBUG] Creating/showing light theme editor")
        if light_theme_edit_frame is None:
            create_light_theme_editor()
        ...
        # Check if dark editor is already open
        if dark_edit_switch.get() and dark_theme_edit_frame:
            # Both open: light goes in column 1
            print("[DEBUG] Both editors open - 50/50 split")
            light_theme_edit_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0))
        else:
            # Only light is open: use column 0 at 50% width
            print("[DEBUG] Only light editor open - 50% width in column 0")
            light_theme_edit_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
    else:
        print("[DEBUG] Hiding light theme editor")
        if light_theme_edit_frame:
            light_theme_edit_frame.grid_forget()
        
        # If dark editor is still open, keep it at 50% width in column 0
        if dark_edit_switch.get() and dark_theme_edit_frame:
            print("[DEBUG] Repacking dark editor to column 0 (50% width)")
            dark_theme_edit_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        else:
            check_container_visibility()
...
def check_container_visibility():
    """Hides the container if both editors are closed"""
    print("[DEBUG] Checking container visibility")
    if not dark_edit_switch.get() and not light_edit_switch.get():
        print("[DEBUG] Both editors closed - hiding container")
        editors_container.pack_forget()

def pick_color(theme_name, color_key, entry_widget, preview_label):
    """Open color picker and update entry"""
    from tkinter import colorchooser
    print(f"[DEBUG] pick_color called for {theme_name}.{color_key}")
    current_color = entry_widget.get()
    color_code = colorchooser.askcolor(
        title=f"Choose {COLOR_LABELS[color_key]}",
        initialcolor=current_color
    )
    
    if color_code[1]:
        print(f"[DEBUG] Color selected: {color_code[1]}")
        entry_widget.delete(0, 'end')
        entry_widget.insert(0, color_code[1])
        preview_label.configure(fg_color=color_code[1])
    else:
        print("[DEBUG] Color picker cancelled")

def create_color_row(parent, theme_name, color_key, color_entries_dict):
    """Create a row for editing a single color"""
    row_frame = ctk.CTkFrame(parent, fg_color="transparent")
    row_frame.pack(fill="x", pady=2)
    
    label = ctk.CTkLabel(
        row_frame,
        text=COLOR_LABELS[color_key],
        width=150,
        anchor="w",
        text_color=colors["text"]
    )
    ...
    preview_label = ctk.CTkLabel(
        ...
        width=30,
        height=25,
        fg_color=THEMES[theme_name][color_key],
        corner_radius=4
    )
    ...
    entry.insert(0, THEMES[theme_name][color_key])
    ...
    pick_btn = ctk.CTkButton(
        ...
        width=60,
        height=25,
        command=lambda: pick_color(theme_name, color_key, entry, preview_label),
        fg_color=colors["accent"],
        hover_color=colors["accent_hover"],
        text_color=colors["accent_text"]
    )
    ...
    color_entries_dict[color_key] = (entry, preview_label)

def create_dark_theme_editor():
    """Create dark theme color editor"""
    global dark_theme_edit_frame, dark_color_entries
    
    print("[DEBUG] create_dark_theme_editor called")
    dark_theme_edit_frame = ctk.CTkFrame(editors_container, fg_color=colors["card_bg"], corner_radius=10)
    
    header_frame = ctk.CTkFrame(dark_theme_edit_frame, fg_color="transparent")
    ...
    reset_btn = ctk.CTkButton(
        ...
        width=120,
        height=28,
        command=lambda: reset_dark_theme(),
    ...
    colors_scroll = ctk.CTkScrollableFrame(
        dark_theme_edit_frame,
        height=300,
        fg_color="transparent"
    )
    colors_scroll.pack(fill="both", expand=True, padx=10, pady=(0, 10))
    
    for color_key in EDITABLE_COLOR_KEYS:
        create_color_row(colors_scroll, "dark", color_key, dark_color_entries)
    
    button_frame = ctk.CTkFrame(dark_theme_edit_frame, fg_color="transparent")
    ...
    apply_btn = ctk.CTkButton(
        ...
        text="Apply Changes",
        height=35,
        command=lambda: apply_dark_theme_changes(),
        fg_color=colors["success"],
        hover_color="#2fc97f",
        text_color="#0a0a0a",
        font=ctk.CTkFont(size=13, weight="bold")
    ...
    print(f"[DEBUG] Dark theme editor created with {len(EDITABLE_COLOR_KEYS)} colors")

def create_light_theme_editor():
    """Create light theme color editor"""
    global light_theme_edit_frame, light_color_entries
    
    print("[DEBUG] create_light_theme_editor called")
    light_theme_edit_frame = ctk.CTkFrame(editors_container, fg_color=colors["card_bg"], corner_radius=10)
    
    header_frame = ctk.CTkFrame(light_theme_edit_frame, fg_color="transparent")
    header_frame.pack(fill="x", padx=10, pady=10)
    ...
    reset_btn = ctk.CTkButton(
        ...
        width=120,
        height=28,
        command=lambda: reset_light_theme(),
        fg_color=colors["warning"],
        hover_color="#e67e22",
        text_color="#0a0a0a"
    )
    ...
    for color_key in EDITABLE_COLOR_KEYS:
        create_color_row(colors_scroll, "light", color_key, light_color_entries)
    
    button_frame = ctk.CTkFrame(light_theme_edit_frame, fg_color="transparent")
    button_frame.pack(fill="x", padx=10, pady=10)
    ...
    apply_btn = ctk.CTkButton(
        ...
        text="Apply Changes",
        height=35,
        command=lambda: apply_light_theme_changes(),
        fg_color=colors["success"],
        hover_color="#2fc97f",
        text_color="#0a0a0a",
        font=ctk.CTkFont(size=13, weight="bold")
    ...
    apply_btn.pack(fill="x")
    print(f"[DEBUG] Light theme editor created with {len(EDITABLE_COLOR_KEYS)} colors")

def reset_dark_theme():
    ...
    if response:
        ...
        if reset_theme_colors("dark"):
            for color_key, (entry, preview) in dark_color_entries.items():
...
def reset_light_theme():
    ...
    if response:
        ...
        if reset_theme_colors("light"):
            for color_key, (entry, preview) in light_color_entries.items():
...
def apply_dark_theme_changes():
    ...
    for color_key, (entry, preview) in dark_color_entries.items():
        ...
        if not color_value.startswith('#') or len(color_value) not in [4, 7]:
            ...
            messagebox.showerror(
                "Invalid Color",
                f"Invalid color code for {COLOR_LABELS[color_key]}: {color_value}\n\nPlease use format #RGB or #RRGGBB"
            )
            return
...
dark_edit_frame = ctk.CTkFrame(settings_scrollable_frame, fg_color="transparent")
...
dark_edit_switch = ctk.CTkSwitch(
    dark_edit_frame,
    text="",
    command=toggle_dark_theme_editor,
    width=50
)
dark_edit_switch.pack(side="left", padx=5)

light_edit_frame = ctk.CTkFrame(settings_scrollable_frame, fg_color="transparent")
...
light_edit_switch = ctk.CTkSwitch(
    light_edit_frame,
    text="",
    command=toggle_light_theme_editor,
    width=50
)
light_edit_switch.pack(side="left", padx=5)
...
def add_carlist_card(carid, name, developer_added=False):
    ...
    lbl = ctk.CTkLabel(
        ...
        anchor="w",
        justify="left",
        font=ctk.CTkFont(size=14, weight="bold"),  # Increased
        text_color=colors["text"]
    )
...
print("[DEBUG] Loading output icons...")
load_output_icons()
print("[DEBUG] Updating output icons for current theme...")
update_output_icons()
...
def show_startup_warning():
    """Show WIP warning dialog - called by main.py after window is initialized"""
    from core.settings import show_wip_warning
    print("[DEBUG] show_startup_warning called from app.py")
    show_wip_warning(app)